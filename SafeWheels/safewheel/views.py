...
def index(request): # Working
    ...
    estabelecimentos = Estabelecimentos.objects.all().order_by("nomeE").values()
    return render(request, 'safewheel/index.html', {"estabelecimentos": estabelecimentos})

def buscar(request):
    buscar = Estabelecimentos.objects.filter(nomeE__icontains=request.POST["buscar"]).values()
    if buscar:
        return HttpResponse('Establecimento Encontrado')
    else:
        return HttpResponse('Estabelecimento não Encontrado')
